...
argument-hint: "PRD path and Story ID. E.g.: tasks/current/prd.json US-003"
---
...
Stories may be executed in **parallel** when they belong to the same `group` in the PRD. **Maximizing parallel execution is critical for speed.** The worker must:

- Be completely independent from other workers executing stories in the same group
- Touch only its own layer/concern (atomic story)
- NOT modify files that might be touched by parallel workers
- Leave the codebase in a valid state (build/lint passes) after completion
- Report its own status without waiting for other workers

### Anti-patterns to avoid:

- ❌ Modifying files outside the story's scope (causes conflicts with parallel workers)
- ❌ Leaving imports broken or types incomplete for other stories
- ❌ Skipping validation steps because "another story will fix it"
...
1. **PRD Path** (`prdPath`): Path to the `prd.json` file or PRD markdown.
   - E.g.: `tasks/current/prd.json` or `tasks/{task-name}/prd.json`
...
```
tasks/
├── current/              # Active task (or tasks/{task-name}/)
│   ├── prd.json          # PRD in Ralph format
│   ├── progress.txt      # Progress log
│   └── prd-[name].md     # PRD in markdown (optional)
```
...
7. **Update AGENTS.md** if reusable learnings are discovered.

8. **Commit changes** with message: `feat: [Story ID] - [Story Title]`

9. **Report result to calling agent:**
...
- `Execute story US-003 from PRD tasks/current/prd.json`
- `Implement US-001 from tasks/current/prd.json`
- `Implement US-001 from tasks/user-authentication/prd.json`

**With custom limits:**

- `Execute US-005 with timeout=60 and maxAttempts=5`
- `Rode US-002 de tasks/current/prd.json (timeout: 15min, 1 tentativa)`
