...
**Execute all pending stories in the current group in parallel, while keeping groups sequential and auditable (A → B → C).**

## Your Role

1. Load and parse the PRD from `prd.json`
2. Identify the next pending group and all incomplete stories inside it
3. Delegate one Sub Agent with `ralph-worker` per incomplete story in the current group
4. Handle all results for the group and continue to the next group
...
2. Update header with:
   ...
   - `Status`: RUNNING
   - Execution Summary: Mark all already-dispatched stories in the current group as in progress
...
2. Update header:
   ...
   - Execution Summary: Update Completed/Failed/Blocked counts and remaining in-progress stories for the current group
    ...
    *** PARALLEL WORKER CALLS ***
    For each story in current group:
        ...
        - Call `ralph-worker` with:
            - `prdPath`: tasks/current/prd.json (or tasks/{task-name}/prd.json)
            - `storyId`: US-XXX
...
1. Check if `tasks/current` exists
2. If `tasks/current` exists, that's the active task
3. If not, look in `tasks/` for folders that do NOT have `[DONE]` prefix
4. If multiple exist, ask the user which to prioritize
5. If none exist, report completion status
...
Stories are executed based on their `group` property. **This is critical — grouping directly determines execution speed.**

- **Same group → Parallel**: Stories with the same `group` value are executed simultaneously (multiple workers at the same time)
...
- **Maximize parallelism**: Every story that can run in parallel is time saved

### Anti-patterns to avoid:

- ❌ Running independent stories sequentially (wastes time)
- ❌ Waiting for one worker before starting another in the same group
- ❌ Skipping the parallel call when a group has multiple stories
...
- PRD Path: tasks/current/prd.json (or tasks/{task-name}/prd.json)
...
- Execute every pending story in the current group before advancing to the next group
